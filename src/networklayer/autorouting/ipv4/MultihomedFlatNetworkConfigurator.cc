...
static void updateIfMetricIsBetter(IRoutingTable* routingTable,
                                   IPv4Route*     newRoute)
{
   IPv4Route* oldRoute = routingTable->findBestMatchingRoute(newRoute->getDestination());
   if(oldRoute) {
   ...
   std::vector<IPv4Address> ownAddresses = routingTable->gatherAddresses();
   for(std::vector<IPv4Address>::iterator iterator = ownAddresses.begin();
       iterator != ownAddresses.end(); iterator++) {
      if(newRoute->getDestination() == *iterator) {
...
static bool nodeFilter(cModule* module, void* userData)
{
   ...
   if(nodeProperty) {
      ...
      IRoutingTable* routingTable = IPvXAddressResolver().routingTableOf(module);
      if(routingTable) {
         if( (parameters->FullTopology) || (routingTable->isIPForwardingEnabled()) ) {
            ...
            IInterfaceTable* interfaceTable = IPvXAddressResolver().interfaceTableOf(module);
            if(interfaceTable) {
                                                        ...
                                                        unsigned int    networkID)
{
   ...
   for(int i = 0; i < topology.getNumNodes(); i++) {
      ...
      nodeInfo[i].isIPNode = (IPvXAddressResolver().findInterfaceTableOf(module) != NULL);
      if(nodeInfo[i].isIPNode) {
         nodeInfo[i].interfaceTable = IPvXAddressResolver().interfaceTableOf(module);
         nodeInfo[i].routingTable   = IPvXAddressResolver().routingTableOf(module);
      }
      ...
      for(int k = 0; k < interfaceTable->getNumInterfaces(); k++) {
         ...
         if(!interfaceEntry->isLoopback()) {
            ...
            EV << "   Interface " << interfaceEntry->getName() << ": "
               << IPv4Address(address).str().c_str() << endl;
            interfaceEntry->ipv4Data()->setIPAddress(IPv4Address(address));
            interfaceEntry->ipv4Data()->setNetmask(IPv4Address::ALLONES_ADDRESS);
         }
      }
   }
   return(networkSet);
...
*/
      ...
      for(int j = 0; j < node->getNumOutLinks(); j++) {
         ...
         IInterfaceTable* neighbourNodeInterfaceTable = IPvXAddressResolver().interfaceTableOf(neighbourNode->getModule());
      ...
      if(nodeInfo[n].usesDefaultRoute) {
         ...
         for (int r = 0;r< nodeRoutingTable->getNumRoutes();r++) {
            const IPv4Route* route = nodeRoutingTable->getRoute(r);
            ev << ++num << "   Route to "
               << route->getDestination().str().c_str() << " via "
   ...
   for(int i = 0;i < topology.getNumNodes();i++) {
      ...
      for (int j = 0; j < topology.getNumNodes(); j++) {
         ...
         for(int k = 0;k < destinationNodeInterfaceTable->getNumInterfaces();k++) {
            ...
            const IPv4Address destinationAddress = interfaceEntry->ipv4Data()->getIPAddress();
