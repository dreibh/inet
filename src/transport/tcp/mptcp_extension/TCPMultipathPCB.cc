...
MPTCP_PCB* MPTCP_PCB::processMPTCPSegment(int connId, int aAppGateIndex,
    ...
    MPTCP_PCB* tmp = MPTCP_PCB::lookupMPTCP_PCB(connId, aAppGateIndex, tcpseg,
            subflow);

    tmp->DEBUGprintFlowOverview(0);
    if (tmp == NULL) {
        DEBUGPRINT(
                "[MPTCP][PROCESS][INCOMING] DID my best, but found no Flow for this subflow %s",
                "\0");
        tmp = new MPTCP_PCB(connId, aAppGateIndex, subflow);

    } else {
        DEBUGPRINT("[MPTCP][PROCESS][INCOMING] Existing flow use exiting PCB%s",
                "\0");
...
int MPTCP_PCB::_processSegment(int connId, TCPConnection* subflow,
    ...
    if (!subflow->getTcpMain()->multipath) {
        ASSERT(
                false
                        && "UPS...We are not in Multipath -> What is wrong here...?"); // FIXME Only for testing
        return 0;
    }
    ...
    if (subflow->flow == NULL || (subflow->flow->getState() == IDLE)
            || (subflow->flow->getState() == PRE_ESTABLISHED)) {
        ...
        for (uint i = 0; i < tcpseg->getOptionsArraySize(); i++) {
            ...
            if (kind == TCPOPTION_MPTCP) {
                ...
                switch (sub) {
                case MP_CAPABLE:
                    DEBUGPRINT("[MPTCP][IDLE][MPTCP OPTION][IN] MP_CAPABLE%s",
                            "\0")
                    ;
                    _processMP_CAPABLE(connId, subflow, tcpseg, &option);
                    break;
                case MP_JOIN:
                    DEBUGPRINT("[MPTCP][IDLE][MPTCP OPTION][IN] MP_JOIN%s",
                            "\0")
                    ;
                ...
                default:
                    DEBUGPRINT(
                            "[MPTCP][IDLE][MPTCP OPTION][IN] Not supported%s",
                            "\0")
                    ;
        ...
        for (uint i = 0; i < tcpseg->getOptionsArraySize(); i++) {
            ...
            if (kind == TCPOPTION_MPTCP) {
                ...
                case MP_CAPABLE:
                    DEBUGPRINT(
                            "[MPTCP][ESTABLISHED][MPTCP OPTION][IN] MP_CAPABLE%s",
                            "\0")
                    ;
                    if ((!tcpseg->getSynBit()) && (tcpseg->getAckBit())) {
                        _processMP_CAPABLE(connId, subflow, tcpseg, &option);
...
#warning "Why we enter this state"
                ...
                case MP_JOIN:
                     ...
                     **/
                    DEBUGPRINT(
                            "[MPTCP][ESTABLISHED][MPTCP OPTION][IN] MP_JOIN%s",
                            "\0")
                    ;
                    _processMP_JOIN_ESTABLISHED(connId, subflow, tcpseg,
                            &option);
                    break;
                case MP_DSS:
                    DEBUGPRINT("[MPTCP][IDLE][MPTCP OPTION][IN] MP_DSS%s", "\0")
                ...
                default:
                    DEBUGPRINT(
                            "[MPTCP][ESTABLISHED][MPTCP OPTION][IN] Not supported%s",
                            "\0")
                    ;
 ...
 *
 */
int MPTCP_PCB::_processMP_CAPABLE(int connId, TCPConnection* subflow,
        TCPSegment *tcpseg, const TCPOption* option) {
 ...
 */
int MPTCP_PCB::_processMP_JOIN_IDLE(int connId, TCPConnection* subflow,
        TCPSegment *tcpseg, const TCPOption* option) {
 ...
 */
int MPTCP_PCB::_processMP_JOIN_ESTABLISHED(int connId, TCPConnection* subflow,
        TCPSegment *tcpseg, const TCPOption* option) {
...
// process SYN
    if ((tcpseg->getSynBit()) && (!tcpseg->getAckBit())) {
        ...
        AllMultipathTCPVector_t::const_iterator it;
        for (it = mptcp_flow_vector.begin(); it != mptcp_flow_vector.end();
                it++) {
 ...
 */
int MPTCP_PCB::_processMP_DSS(int connId, TCPConnection* subflow,
        TCPSegment *tcpseg, const TCPOption* option) {
     ...
     */
    //
    DEBUGPRINT("[FLOW][RCV][DSS][STATUS] snd_una: %ld",
            subflow->flow->mptcp_snd_una);DEBUGPRINT(
            "[FLOW][RCV][DSS][STATUS] snd_nxt: %ld",
            subflow->flow->mptcp_snd_nxt);DEBUGPRINT(
            "[FLOW][RCV][DSS][STATUS] snd_wnd: %d",
            subflow->flow->mptcp_snd_wnd);DEBUGPRINT(
            "[FLOW][RCV][DSS][STATUS] rcv_nxt: %ld",
            subflow->flow->mptcp_rcv_nxt);DEBUGPRINT(
            "[FLOW][RCV][DSS][STATUS] rcv_wnd: %ld",
            subflow->flow->mptcp_rcv_wnd);
    ...
    DEBUGPRINT(
            "[FLOW][DSS][INFO][RCV] Ack Seq: %ld \t SND Seq: %ld \t Subflow Seq: %d \t Data length: %d",
...
int MPTCP_PCB::_processMP_ADD_ADDR(int connId, TCPConnection* subflow, TCPSegment *tcpseg,const TCPOption* option) {
    ...
    if (option->getLength()>12) {
        raddr->addr = IPv6Address(option->getValues(1), option->getValues(2), option->getValues(3), option->getValues(4));
        raddr->port = option->getValues(5);
    } else {
...
MPTCP_PCB* MPTCP_PCB::_lookupMPTCP_PCB(int connId, int aAppGateIndex,
    ...
    for (it = mptcp_flow_vector.begin(); it != mptcp_flow_vector.end(); it++) {
        ...
        TCP_SubFlowVector_t::const_iterator it_mpflows;
        if (t->flow != NULL) {
            for (it_mpflows = t->flow->getSubflows()->begin();
                    it_mpflows != t->flow->getSubflows()->end(); it_mpflows++) {
                TCP_subflow_t* sub = (TCP_subflow_t*) (*it_mpflows);
                if ((sub->subflow->connId == connId)
                        && (sub->subflow->appGateIndex == aAppGateIndex)) {
                    if (t->flow->getLocalToken() != 0
                            && t->flow->getRemoteToken() != 0)
        ...
        if (kind == TCPOPTION_MPTCP) {
            ...
            if (sub == MP_CAPABLE) {
                if ((!tcpseg->getSynBit()) && (tcpseg->getAckBit())) { // It is an ACK we have to know this
                    ...
                    AllMultipathTCPVector_t::const_iterator it;
                    for (it = mptcp_flow_vector.begin();
                            it != mptcp_flow_vector.end(); it++) {
                        TuppleWithStatus_t* t = (TuppleWithStatus_t *) (*it);
 ...
 */
MPTCP_PCB* MPTCP_PCB::_lookupMPTCPbySubflow_PCB(int connId, int aAppGateIndex,
        TCPSegment *tcpseg, TCPConnection* subflow) {
    ...
    for (it = mptcp_flow_vector.begin(); it != mptcp_flow_vector.end(); it++) {
        ...
        if ((t->appGateIndex == aAppGateIndex) && (t->connID == connId)) {
            DEBUGPRINT("APP GATE UND ConnID sind gleich %i==%i %i==%i",
                    t->appGateIndex, aAppGateIndex, t->connID, connId);
            if (t->flow->isSubflowOf(subflow)) {
                subflow->flow = t->flow;
                return t->flow->getPCB();
            }
...
MPTCP_PCB* MPTCP_PCB::lookupMPTCP_PCB(int connId, int aAppGateIndex,
    ...
    if (tmp == NULL) {
        ...
        if ((uint32) subflow->getTcpMain()->multipath_subflow_id != 0) {
            ...
            for (it = mptcp_flow_vector.begin(); it != mptcp_flow_vector.end();
                ...
                TuppleWithStatus_t* t = (TuppleWithStatus_t *) (*it);
                if (t->flow != NULL) {
                    if (((uint32) subflow->getTcpMain()->multipath_subflow_id)
                            == t->flow->getLocalToken()) {
    ...
    if (tmp == NULL) {
        DEBUGPRINT(
                "[MPTCP][PROCESS][INCOMING]_lookupMPTCP_PCBbyMP_JOIN_Option%s",
                "\0");
    ...
    if (tmp == NULL)
        DEBUGPRINT("[IDLE][lookupMPTCP_PCB] found no PCB for %i and %i", connId,
                aAppGateIndex);
...
void MPTCP_PCB::DEBUGprintFlowOverview(int type) {
    ...
    for (it = mptcp_flow_vector.begin(); it != mptcp_flow_vector.end(); it++) {
        ...
        case IDLE:
            ...
            ;
            DEBUGPRINT(
                    "[MPTCP][OVERVIEW][PCB][FLOW] Flow ID TOKEN Local  %u IDLE",
                    tmp->flow->getLocalToken())
            ;
            DEBUGPRINT(
                    "[MPTCP][OVERVIEW][PCB][FLOW] Flow ID TOKEN REMOTE  %u IDLE",
                    tmp->flow->getRemoteToken())
            ;
            break;
        case PRE_ESTABLISHED:
            DEBUGPRINT(
                    "[MPTCP][OVERVIEW][PCB][FLOW] Flow ID TOKEN Local  %u PRE_ESTABLISHED",
                    tmp->flow->getLocalToken())
            ;
            DEBUGPRINT(
                    "[MPTCP][OVERVIEW][PCB][FLOW] Flow ID TOKEN REMOTE  %u PRE_ESTABLISHED",
                    tmp->flow->getRemoteToken())
            ;
            break;
        case SHUTDOWN:
            DEBUGPRINT(
                    "[MPTCP][OVERVIEW][PCB][FLOW] Flow ID TOKEN Local  %u SHUTDOWN",
                    tmp->flow->getLocalToken())
            ;
            DEBUGPRINT(
                    "[MPTCP][OVERVIEW][PCB][FLOW] Flow ID TOKEN REMOTE  %u SHUTDOWN",
                    tmp->flow->getRemoteToken())
            ;
            break;
        case ESTABLISHED:
            ...
            ;
            {
                ...
                case 0:
                    DEBUGPRINT(
                            "[MPTCP][OVERVIEW][PCB][FLOW] SERVER INSTANCE Flow ID TOKEN Local  %u <--> REMOTE %u ESTABLISHED - Flow Token %u",
                            tmp->flow->getLocalToken(),
                            tmp->flow->getRemoteToken(),
                            tmp->flow->getPCB()->id)
                    ;
                ...
                default:
                    DEBUGPRINT(
                            "[MPTCP][OVERVIEW][PCB][FLOW] CLIENT APP %u   Flow ID TOKEN Local  %u <--> REMOTE %u ESTABLISHED - Flow Token %u",
                            tmp->flow->getAppID(), tmp->flow->getLocalToken(),
                            tmp->flow->getRemoteToken(),
                            tmp->flow->getPCB()->id)
                    ;
                }
            ...
            DEBUGPRINT(
                    "[MPTCP][OVERVIEW][PCB][FLOW] Base Sequence Number:        %ld",
                    tmp->flow->getBaseSQN())
            ;
